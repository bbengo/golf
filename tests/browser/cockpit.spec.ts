...
test('QR pairing, live controls, shot, phone reconnect, and display isolation', async ({
   ...
   await screen.locator('#openGameOptions').click();
   await screen.locator('[name=displayMode][value=desktop]').check();
   await screen.locator('#closeGameOptions').click();
   await expect(screen.locator('#desktopClub')).toHaveValue('6I');
   await expect(screen.locator('#desktopEffort')).toHaveValue('85');
   await screen.locator('#desktopEffort').fill('80');
   await expect(phone.locator('#effort')).toHaveValue('80');
   await phone.locator('#effort').fill('85');
   await expect(screen.locator('#desktopEffort')).toHaveValue('85');
   await screen.locator('#revealTools').click();
   await screen.locator('#openGameOptions').click();
   await screen.locator('[name=displayMode][value=clear]').check();
   await screen.locator('#closeGameOptions').click();
