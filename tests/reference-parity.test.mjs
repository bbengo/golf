...
const cases = [
  ...
  { name: 'shaped iron in dry conditions', club: '6I', start: 'tee', moisture: 'dry', shape: 0.4, height: -0.3 },
  { name: 'wedge in wet conditions', club: 'PW', start: 'approach', moisture: 'wet', shape: -0.25, height: 0.5 },
  { name: 'chip toward green', club: 'CHIP', start: 'approach', moisture: 'moderate', shape: 0, height: 0 },
  { name: 'putt on green', club: 'PUTT', start: 'greenStart', moisture: 'moderate', shape: 0, height: 0 },
];
for (const fixture of cases) {
  test(`reference parity: ${fixture.name}`, () => {
    ...
    const intent = {
      ...
      aim: plain(course.pin), club: fixture.club, effort: 0.9,
      shape: fixture.shape, height: fixture.height, variance: true,
    };
    ...
    const expected = reference.Shot.run(plain(course), plain(day), plain(golfer), plain(intent), 12345);
